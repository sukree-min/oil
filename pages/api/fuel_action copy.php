...
try {
    switch ($action) {
        ...
        // ============================================
        case 'get_vehicles':
            $stmt = $conn_kkdoc->query("SELECT * FROM oil_vehicles WHERE is_active = 1 ORDER BY plate_number");
        ...
        // ============================================
        case 'get_drivers':
            $stmt = $conn_kkdoc->query("SELECT * FROM oil_drivers WHERE is_active = 1 ORDER BY driver_name");
            echo json_encode(['status' => 'success', 'data' => $stmt->fetchAll()]);
            break;
        ...
        case 'add_record':
            ...
            echo json_encode(['status' => 'success', 'message' => 'บันทึกข้อมูลสำเร็จ', 'id' => $conn_kkdoc->lastInsertId()]);
            break;
        ...
        case 'update_record':
            ...
            // จัดการรูปภาพบิลเดิม
            $stmt = $conn_kkdoc->prepare("SELECT bill_image FROM oil_fuel_records WHERE id = ?");
            $stmt->execute([$id]);
        ...
        case 'delete_record':
            ...
            $stmt = $conn_kkdoc->prepare("DELETE FROM oil_fuel_records WHERE id = ?");
            $stmt->execute([$id]);
        ...
        case 'add_vehicle':
            ...
            $stmt = $conn_kkdoc->prepare("INSERT INTO oil_vehicles (plate_number, vehicle_name, vehicle_type, image) VALUES (?, ?, ?, ?)");
        ...
        case 'update_vehicle':
            ...
            $stmt = $conn_kkdoc->prepare("SELECT image FROM oil_vehicles WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $stmt = $conn_kkdoc->prepare("UPDATE oil_vehicles SET plate_number = ?, vehicle_name = ?, vehicle_type = ?, image = ? WHERE id = ?");
        ...
        case 'delete_vehicle':
            ...
            $stmt = $conn_kkdoc->prepare("SELECT COUNT(*) FROM oil_fuel_records WHERE vehicle_id = ?");
            $stmt->execute([$id]);
            if ($stmt->fetchColumn() > 0) {
                $stmt = $conn_kkdoc->prepare("UPDATE oil_vehicles SET is_active = 0 WHERE id = ?");
                $stmt->execute([$id]);
                ...
                $stmt = $conn_kkdoc->prepare("DELETE FROM oil_vehicles WHERE id = ?");
                $stmt->execute([$id]);
        ...
        case 'add_driver':
            ...
            $stmt = $conn_kkdoc->prepare("INSERT INTO oil_drivers (driver_name, phone, image) VALUES (?, ?, ?)");
        ...
        case 'update_driver':
            ...
            $stmt = $conn_kkdoc->prepare("SELECT image FROM oil_drivers WHERE id = ?");
            $stmt->execute([$id]);
            $old_image = $stmt->fetchColumn();
            $image = $old_image;
            ...
            $stmt = $conn_kkdoc->prepare("UPDATE oil_drivers SET driver_name = ?, phone = ?, image = ? WHERE id = ?");
        ...
        case 'delete_driver':
            ...
            // ตรวจสอบว่ามีการใช้งานอยู่ในรายการน้ำมันหรือไม่
            $stmt = $conn_kkdoc->prepare("SELECT COUNT(*) FROM oil_fuel_records WHERE driver_id = ?");
            $stmt->execute([$id]);
            if ($stmt->fetchColumn() > 0) {
                $stmt = $conn_kkdoc->prepare("UPDATE oil_drivers SET is_active = 0 WHERE id = ?");
                $stmt->execute([$id]);
                ...
                $stmt = $conn_kkdoc->prepare("DELETE FROM oil_drivers WHERE id = ?");
                $stmt->execute([$id]);
